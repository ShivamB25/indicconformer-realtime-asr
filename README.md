...
For the deeper ownership map, runtime race semantics, research decisions, and deployment runbook, see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
VAD inference has a two-phase cancellation contract. A deadline or caller
cancellation can prevent a job that is still queued in the executor from
starting; it cannot safely preempt an already-running `asyncio.to_thread` or
ONNX Runtime call. Once started, the worker retains job-owned input/state
snapshots until return and discards stale results by Future/epoch checks.
The private `_OUTPUT_NAMES` list is kept in the existing list form and is never
mutated while `session.run` is in flight; `Final` is a static annotation, not
runtime immutability. The complete ownership and race record is in
